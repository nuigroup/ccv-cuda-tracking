...
float gpu_DetectBlob( unsigned char *in, unsigned char *labels)
{
	...
	cudaMalloc( (void **)&gpu_in, imageW * imageH);
	cudaMemcpy( gpu_in, in, imageW * imageH, cudaMemcpyHostToDevice);
	...
	cudaMemcpy( labels, gpu_labels_uchar, imageW*imageH, cudaMemcpyDeviceToHost);
